...
{{--                        </a>--}}
        ...
        <div class="content content-full content-boxed">
            ...
            <hr>
                ...
                @endif
                @if(session()->has('profit'))
                    <div class="alert alert-success">
                        {{ session()->get('profit') }}
                    </div>
                @endif
            <form action="{{ route('admin.editBalance') }}" method="POST">
                @csrf
                <input type="hidden" value="{{ $user->id }}" name="user_id">

                <div class="row">
                    <div class="col-lg-6">
                        <input type="text" name="balance" class="form-control" value="{{ old('balance', optional($user)->balance) }}">
                    </div>
                    <div class="col-lg-6">
                        <button class="btn btn-primary">Update Balance</button>
                    </div>
                </div>
            </form>
                <br>
            <form action="{{ route('admin.editProfit') }}" method="POST">
                @csrf
                <input type="hidden" value="{{ $user->id }}" name="user_id">

                <div class="row">
                    <div class="col-lg-6">
                        <input type="text" name="profit" class="form-control" value="{{ old('profit', optional($user)->profit) }}">
                    </div>
                    <div class="col-lg-6">
                        <button class="btn btn-primary">Update Profit</button>
                    </div>
                </div>
            </form>
                <br>
